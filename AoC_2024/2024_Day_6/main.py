...
def start_point(lines):
    for row in range(len(lines)):
        for col in range(len(lines[1])):
            if lines[row][col] == "^":  # Compare the character at (y, x)
                return row,col



def keeping_moving(lines, position_row, position_col, visited):
        
    while (lines[position_row][position_col]) != "#":
        if facing == 'up':
            position_row -=1
        if facing == 'down':
            position_row +=1
        ...
        if facing == 'right':
            position_col +=1
        visited.add((position_row, position_col))
        print(facing)
    ...
    return facing, position_row, position_col 
...
def part1(data):
    ...
    visited = set()
    list2 = []
    position_row, position_col= start_point(lines)
    visited.add((position_row, position_col))
    list2.append((position_row, position_col))

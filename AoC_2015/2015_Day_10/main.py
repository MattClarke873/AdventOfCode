...
def part1(data):
    """Solve problem one."""
    number = "1321131112"  # Start with a string
    count = 0  
    ...
    while count < 40:
        ...
        while i < len(number):
            digit = number[i]
            run_length = 1  # Count occurrences of the same digit

            # Count consecutive occurrences of digit
            while i + 1 < len(number) and number[i + 1] == digit:
                run_length += 1
                i += 1

            # Append count and digit to answer
            answer += str(run_length) + digit
            i += 1  # Move to next unique digit

        number = answer  # Update number
        count += 1

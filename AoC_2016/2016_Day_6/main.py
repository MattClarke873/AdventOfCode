...
def part1(data):
    ...
    for char in range(len(lines[0])):
        ...
        check_set = set()
        for line in lines:
            col.append(line[char])
            if line[char] not in check_set:
                check_set.add(line[char])
        for i in check_set:

            
            signal[i] = col.count(i)
        
        final_signal += max(signal, key=signal.get)
    print(final_signal)
...
def part2(data):
    ...
    for char in range(len(lines[0])):
        ...
        final_signal += min(signal, key=signal.get)
    print(final_signal)

...
def part1(data):
    ...
    while len(password) < 8 :
        string = data + str(i) 
        answer_bytes = string.encode()
        h = hashlib.md5(answer_bytes)
        answer = h.hexdigest()
        answer = str(answer)
        i += 1
        if answer.startswith("00000"):
            password.append(answer[5])
...
def part2(data):
    ...
    while len(password_index) < 8 :
        ...
        if answer.startswith("00000"):
            if answer[5].isdigit():
                if answer[5] not in password_index and 0 <= int(answer[5]) <= 7:
                    password_index.add(answer[5])
                    
                    password[int(answer[5])] = answer[6]
                    print(f'index {answer[5]} char {answer[6]}')
                else:
                    pass

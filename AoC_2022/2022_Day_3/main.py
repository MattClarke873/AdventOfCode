...
def split_sacks(sack):
    length = len(sack)
    half = int(length/2)
    comp1 = sack[:half]
    comp2 = sack[half:]
    #print(f'Comp1 {comp1:<25} Comp2  {comp2}')
    return comp1,  comp2

def double(comp1, comp2):
    for item in comp1:
        if item in comp2:
            return item

def three_man(elf1:str, elf2:str, elf3:str):
    for item in elf1:
...
def part1(data):
    ...
    answer = 0
    for sack in sacks:
        comp1, comp2 = (split_sacks(sack))
        
        item = (double(comp1, comp2))
        
        answer += (char_to_int(item))
...
def part2(data):
    ...
    for sack in range(0, len(sacks), 3):
        ...
        item = (three_man(sacks[sack], sacks[sack+1], sacks[sack+2]))
